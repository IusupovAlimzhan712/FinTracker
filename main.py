...
def main():
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            tab1, tab2 = st.tabs(["Expences (Debits)", "Playments (Credits)"])
            with tab1:
                ...
                st.session_state.debits_df["Date"] = (
                    st.session_state.debits_df["Date"]
                    .astype(str)        
                    .str.strip()        
                )

                st.session_state.debits_df["Date"] = pd.to_datetime(
                    st.session_state.debits_df["Date"],
                    errors="coerce",
                ...
                st.session_state.debits_df["Amount"] = (
                    st.session_state.debits_df["Amount"]
                    .replace(",", "", regex=True)
                    .astype(float)
                )
                ...
                edited_df = st.data_editor(
                    ...
                    column_config = {
                        ...
                        "Amount": st.column_config.NumberColumn("Amount", format = "%.2f MYR"),
            ...
            with tab2:
                ...
                st.metric("Total Payments", f"{total_payments:,.2f} MYR")
                st.write(credits_df)
